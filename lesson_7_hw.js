 ...
 */
    ...
    function stringToArray(str) {
        ...
        const cleanArray = wordsArray.filter(word => word !== '');
        const lowerCaseArray = cleanArray.map(word => word.toLowerCase());
        lowerCaseArray.sort();
...
function removeDuplicatesAndSort(arr) {
    ...
    //Решение 1
    // uniqueArray.sort((a, b) => a - b);   //в этом случа выводит с тройным пробелом перед 6 и я не пойму как от него избавиться
    // return uniqueArray; 

    //Решение 2
    const cleanArray = uniqueArray.filter(a => a !== ''); 
    cleanArray.sort((a, b) => a - b); 
    return cleanArray.join(', '); // вот так выводит красиво, но большой минус джойна, как я понял, это то что он преобразовывает числовые значения в стрингу
}
